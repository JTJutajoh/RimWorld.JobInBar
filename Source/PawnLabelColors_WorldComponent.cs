...
namespace JobInBar
{
    // Disabling for now as I want to replace this entire system with something new.
    ...
    //    {
    //        if (pawn != null && (PawnNameColors.ContainsKey(pawn) || PawnJobColors.ContainsKey(pawn)))
    //        {
    ...
    //    {
    //        if (pawn != null && PawnShowJobLabels != null && PawnShowJobLabels.ContainsKey(pawn))
    //        {
    //            return PawnShowJobLabels[pawn];
    //        }
    //        else
    //        {
    //            Log.WarningOnce($"[Dark.JobInBar] failed to check if a pawn's job should be drawn or not. (Was the pawn deleted?)", 239487);
